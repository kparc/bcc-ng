...
ZK M[31];J AWS=-32;//!< M memory buckets, W wssize (W0 initial offset for GT header and seed alloc c0() \see init()
...
J ws(){R AWS;}K1(r1){P(Ax,x)R++xr?x:O("r1");}
V1(r0){//O("r0 %llx Ax %d xn %d\n",x,Ax,!Ax?xn:0);
 if(Ax||!x||KS==xt)R;if(8==xt){l0(x);R;}if(xr){--xr;R;}if(!xt||KS<xt)N1(xn,r0(Xx))AWS-=16L<<xm,xx=M[xm],M[xm]=x;}
K tn(I t,I n){K x=m1(MX(1,n)*nt[KS<t?0:t]);R AWS+=16L<<xm,xu=0,xt=t,xn=n,x;}
...
V*aw_realloc(V*p,size_t n){K x=(K)p;                  //!< if ptr is null, realloc() is same as malloc
    P(!x||!n,$(x,aw_free(p));aw_malloc(n?n:1))        //!< if size is 0 and ptr is not, ptr is freed and a new 1b-sized object is allocated
    P(KC-xt,("`nyi"),(V*)0)                           //!< realloc only supports char-typed lists
...
V*aw_calloc(size_t n,size_t sz){                      //!< allocate n*sz bytes and fill with zeroes:
